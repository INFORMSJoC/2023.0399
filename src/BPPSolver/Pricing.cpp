...
Pricing::Pricing(CPM* cpm, vi& freq, vi& conflictIts, vll& v, vector<vc>& compatible):
...
void Pricing::recoverDPLexic(int i, Pattern* pat){
  ...
  for(int it : pat->its)
    if(freqRet[it] > 2 * mxFreqRet)
      return;
  if(IMPROVE(dp[i - 1][pat->residue()] + pat->val))
    recoverDPLexic(i - 1, pat);
}
...
vector<Pattern*> Pricing::DP(bool& opt, bool& feasible){
  ...
  if(retDP.empty()){
    if(goodAll){
      ...
      while(i > 0 && w >= 0){
        int it = its[i];
        if(items[it].w <= w && dp[i][w] == (dp[i - 1][w - items[it].w] - v[it])){
          val -= v[it];
          w -= items[it].w;
          cout << it << " - v: " << DD(v[it]) << " - w: " << items[it].w << " - V: "  <<  DD(val) << " - W: " << w  << " - i: " << i << endl;
        }
        i--;
      }
...
vector<Pattern*> Pricing::Solve(bool& opt, bool& feasible) {
  ...
  stable_sort(all(its), [&](int i1, int i2){
    return prior[i1] != prior[i2] ? prior[i1] < prior[i2]: items[i1].w < items[i2].w;
  });
  ret = DP(opt, feasible);
...
ll Pricing::MinReducedCost(){
  ...
  while(!q.empty()){
    ...
    if(BEST_THAN(dp[i - 1][pat->residue()] + pat->val, best->val))
      q.push({i - 1, pat, pos.seen});
    else
      delete pat;
  }
  if(best->sz())
