...
Model::Model(Instance* instance): instance(instance),
  ...
  weights(instance->weights), freq(instance->freq), wToIt(2 * binCapacity, -1),
  ...
  pricing = new Pricing(cpm, weights, freq, wToIt, conflictIts, dual, compatible, insideGT);
...
void Model::initModel(){
  try{
    ...
    FOR(i, N)
      model->addConstr(GRBLinExpr(), GRB_LESS_EQUAL, freq[i], "u" + to_string(i));
    model->update();
...
void Model::getDual(){
  ...
  safeObj = ((__int128) obj * (BASIS + epsM)) / BASIS;
}
...
bool Model::colGen(){
  ...
  ld bef = UpdateTime();
  pricing->gtSet = insideGT ? constrsGT[0].ids : si();
...
void Model::rootRuleOptim(){
  if(numDualIneq == N){
    ...
    if(good){
      ...
      FOR(i, N){
        ld RHS = floor(multiW * weights[i] * 1e9) / binCapacity / 1e9;
  ...
  FOR(i, M){
    ...
    if(!insideGT){
      if((primal[i] > EPS9 && !(pricing->UpdateVal(pat) > -epsM)) || 
        ...
        cout << "NumericError: " << primal[i] << " " << DD(pricing->UpdateVal(pat)) << endl;
        if(!firstTry)
          assert(false);
        firstTry = false;
        FastPL();
...
void Model::ShrinkRA(){
  if (features.count("Waste"))
    pricing->ClearPool(min<int>(2 * binCapacity - 1, floor(max<ld>(EPS10, (L0 - bestInteger + EPS10)) * binCapacity)));
}
...
bool Model::AddCuts(ll& curObj){
  ...
  /*if(!features.count("Cut"))
    return false;
  int cnt = 0;
  while(cnt++ < 10){ // && cpm->GetActCuts() < MAX_CUT_ALLOWED
    CalcAfim();
...
ll Model::RunPL(){
  try {
    ...
    if(!bounded){
      ...
      Pattern::dfval = 0;
      GRBColumn col;
      col.addTerm(1.0, constrsGT[0].grbConstr);
...
void Model::AddIts(const vi& its, vi pats, bool force, bool improveIncumb, bool gt){
  ...
  for(int it : its2){
    ...
    if(freq[it] == f){
      model->addConstr(linExp, GRB_LESS_EQUAL, freq[it], "u" + to_string(it));
      N++;
    }else{
      auto constr = model->getConstrByName("u" + to_string(it));
...
bool Model::ResidueOptim(WasteRule rule){
  if(optimal || !features.count("Waste"))
    return false;
  if(rule == OPTIMIZE)
    pricing->capAccepted = max(binCapacity, min<int>(2 * binCapacity - 1, floor(binCapacity * (L0 - bestInteger + EPS10))));
  else{
    if(rule == ROOT){
      if(!bounded)
        return false;
      pricing->capAccepted = min<int>(pricing->capAccepted, floor(binCapacity * (1 + L0 - min<ld>(L1, DD(obj)) + EPS10)));
    }
  }
  assert(pricing->capAccepted >= binCapacity && pricing->capAccepted <= 2 * binCapacity - 1);

  vi toRem;
  for(auto pat : actPatterns)
    if(pat->getCap() > pricing->capAccepted)
      toRem.push_back(pat->id);
