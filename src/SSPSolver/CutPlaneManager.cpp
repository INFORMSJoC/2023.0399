...
CPM::CPM(GRBModel*& model, GRBVar*& vars, const int& N, bool& bounded, const vi& weights,
  const vi& freq, const vi& wToIt, const vi& ids, const vd& primal,
  const vll& dual, const vector<vd>& afim, const vector<set<int>>& itToIds,
  map<int, int>& invIds):
    model(model), vars(vars), N(N), bounded(bounded), weights(weights), freq(freq),
    wToIt(wToIt), ids(ids), primal(primal), dual(dual), afim(afim), itToIds(itToIds),
    invIds(invIds), itsToSRAll(N, vector<IM>()), itsToSR(N, vector<IM>()){}

void CPM::updateItemList(){
  if(SZ(itsToSR) < SZ(weights)){
    itsToSR.resize({});
    itsToSRAll.resize({});
  }
}
...
vector<IM> CPM::getIdsCut(Cut cut){
  ...
  vector<IM> ret;
  for(auto pr : acumIdx)
    ret.push_back({pr.first, pr.second / cut.d + (pr.second % cut.d != 0)});
  return ret;
}
...
multimap<ld, pair<Cut, vector<IM>>, greater<ld>> CPM::calcCuts(){
  vector<vi> rel(SZ(weights));
  FOR(it1, SZ(weights))
    if(freq[it1] == 1)
      for(int it2 = it1 + 1; it2 < SZ(weights); it2++)
      ...
      for(int it3 : ls){
        ...
        if(val < 1 - EPS8){
          ...
          if(valIneq.first < cut.RHS - EPS8)
            cuts.insert({cut.RHS - valIneq.first, {cut, valIneq.second}});
        }
      }
    }
  }
...
bool CPM::GenCuts(){
  ...
  for(auto& pr : cuts){
    ...
    model->addConstr(constr, GRB_GREATER_EQUAL, cut.RHS, "c" + to_string(id));
  }
  ...
  FOR(id, SZ(setsSRAll)){
    if(mapSR[id] >= -1){
      ...
      for(IM im : setsSRAll[id])
        good &= freq[im.i] == 1 && !itToIds[im.i].empty();
