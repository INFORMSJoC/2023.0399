...
Solution* RoundingHeuristic::roundBFD(vector<pair<ld, vi>>& fratBins, Solution* partial, Instance* instance, int target){
  ...
  vi freq2 = freq;
  int SA = round((instance->L0 - target) * binCapacity);
  ...
  for(auto& pr : fratBins){
    ...
    FOR(i, SZ(bin)){
      ...
      any |= freq2[it] > 0;
      cap += min(freq2[it], f) * weights[it];
    }

    if(any && cap >= binCapacity && (cap - binCapacity) <= SA){
      SA -= (cap - binCapacity);
  ...
  FOR(it, SZ(freq2))
    while(freq2[it] > 0){
      ws.push_back(weights[it]);
      freq2[it]--;
    }
  if(!ws.empty())
    sol->VectorPackingBFD(ws);
