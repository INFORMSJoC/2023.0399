...
bool BranchManager::branchOnEnd(int it){
  if(it == -1){
    vd ended(SZ(items));
    FOR(i, model->M){
      Pattern* pat = idToPat[ids[i]];
      ended[pat->its[0]] += min(1.0, model->primal[i]);
    }
    FOR(i, SZ(ended))
      ended[i] = min(1.0, ended[i]);
    vector<pair<ld, int>> vans;
    FOR(it, SZ(items)){
      ld val = fabs(round(ended[it]) - ended[it]);
      if(val > 1e-8)
        vans.push_back({ended[it], it});
    }
    if(vans.empty())
      return false;
    it = max_element(all(vans))->second;
  }
  assert(model->canMid[it]);
  model->canMid[it] = false;
  st.push_back(Node(it));
  lvl++;
  vi t;
  st.back().split = model->UpdateIt(it, t);
  return true;
}
...
bool BranchManager::mergeIts(int it1, int it2){
  if(it1 == -1){
    ...
    vector<tuple<int, int, int, ii>> vans;
    FOR(it1, SZ(items))
      FOR(it2, SZ(items)){
        ...
        if(freq[it1] > 0 && freq[it2] > (it1 == it2) && items[it1].w >= items[it2].w){
          ...
          vans.push_back({dp, abs(items[it1].p - items[it2].p), w, ii(it1, it2)});
        }
      }
...
void BranchManager::swapState(){
  ...
  if(node.onEnd){
    model->canEnd[it1] ^= 1;
    model->canMid[it1] ^= 1;
    node.split = model->UpdateIt(it1, node.split);
    return;
  }
...
int BranchManager::backtracking(){
  ...
  if (features.count("Splay"))
    if(!st.back().onEnd)
      REV(i, SZ(st) - 1){
        if(st[i].fixed || st[i].state || st[i].onEnd)
          break;
        rem[i] = true;
      }
...
void BranchManager::remPosStack(int p){
  ...
  if(node.onEnd){
    model->canEnd[it1] = model->canMid[it1] = true;
    model->UpdateIt(it1, node.split);
  }else{
    if(node.state)
...
bool BranchManager::FixST(ll& curObj, bool print){
  ...
  if(!st.back().state && !st.back().onEnd){
    int it1 = st.back().it1, it2 = st.back().it2;
    auto iter = find(all(globalPrior), ii(it1, it2));
    if(iter != globalPrior.end()){
