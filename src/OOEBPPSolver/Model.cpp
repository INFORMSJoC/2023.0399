...
Model::Model(Instance* instance): instance(instance),
  ...
  items(Pattern::items), freq(instance->freq), canMid(SZ(freq), 1), canEnd(SZ(freq), 1),
  ...
  pricing = new Pricing(cpm, freq, canMid, canEnd, conflictIts, dual, compatible);
...
bool Model::colGen(){
  ...
  if(!opt){
    ...
    // if(bounded)
    //   safeObj = max(safeObj, lagrangianDualBound());
    optPricing = true;
  }
  return opt;
}
...
int Model::AppendIt(int it1, int it2){
  ...
  canMid.push_back(1);
  canEnd.push_back(canEnd[it2]);

  items.push_back(Item(items[it1].w + items[it2].w, items[it1].w + items[it2].wi, items[it2].p));
...
vi Model::UpdateIt(int it, vi& toAdd){
  model->update();
  AddPatternList(toAdd);
  vi toRem;
  for(int id : itToIds[it]){
    if(!pricing->IsCompatible(idToPat[id]))
      toRem.push_back(id);
  }
  RemPatternList(toRem);
  return toRem;
}
