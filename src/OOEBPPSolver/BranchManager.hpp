...
struct Node {
  ...
  bool state, fixed, onEnd;

  Node(int it1, int it2): it(-1), it1(it1), it2(it2), state(false), fixed(false), onEnd(false){}

  Node(int it1): it(-1), it1(it1), it2(-1), state(false), fixed(false), onEnd(true){}
};
