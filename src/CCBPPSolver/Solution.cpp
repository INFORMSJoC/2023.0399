...
void Solution::Check() const{
  ...
  for(Pattern* bin : sol){
    ...
    for(int id : bin->its){
      ...
      colors[Pattern::items[id].c[0]]++, nc += colors[Pattern::items[id].c[0]] == 1;
      freqCnt[id]++;
    }
    assert(cap  <= binCapacity && nc <= C);
  }
  FOR(i, n)
...
void Solution::PrintSolution(string pathOutSol, const StatusSolution& statusSol){
  ...
  for(Pattern* bin: sol){
    ...
    for(int id : bin->its){
      outSolStream << "Size: " << Pattern::items[id].w << " Color: " << Pattern::items[id].c[0] << endl;
...
void Solution::BFD(){
  ...
  sort(all(toPack), [items = Pattern::items](int i1, int i2){
    return items[i1].w > items[i2].w;
  });
