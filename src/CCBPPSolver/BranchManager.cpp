...
bool BranchManager::mergeIts(int it1, int it2){
  if(it1 == -1){
    ...
    vector<tuple<int, bool, int, ii>> vans;
    FOR(it1, SZ(items))
      FOR(it2, SZ(items)){
        ...
        if(valF < 1e-3)
          continue;
        vi tmp;
        set_union(rall(items[it1].c), rall(items[it2].c), back_inserter(tmp));
        bool eq = tmp == (SZ(items[it1].c) > SZ(items[it2].c) ? items[it1].c : items[it2].c);
        if(freq[it1] > 0 && freq[it2] > (it1 == it2) && items[it1].w >= items[it2].w){
          ...
          vans.push_back({dp, eq, w, ii(it1, it2)});
        }
      }
