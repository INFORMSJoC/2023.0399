...
void Model::Reset(int cap){
  ...
  bounded = optimal = masterCol = insideRF = insideGT = false;
  rfHeuristic = optPricing = false;
  lastObj = INFLL;
  L2 = 0;

  binCapacity = cap;
  pricing->residueAccept = binCapacity - 1;
  pricing->W = binCapacity;
  useInitialHeuristic();
  model->update();
  if(bestInteger <= numMachines)
    return void(optimal = true);

  pricing->UpdateDPsize(N, true);

  instance->computeLBs();
  if(SZ(wToIt) <= binCapacity)
    wToIt.resize(binCapacity + 1, -1);
  ...
  vi toRem;
  for(auto pat : actPatterns)
    if(pat->cap > binCapacity)
      toRem.push_back(pat->id);

  RemPatternList(toRem);
  FOR(i, SZ(instance->freq))
    assert(freq[i] == instance->freq[i]);

  if(bestInteger > numMachines + 1){
    bestInteger = numMachines + 1;
    delete bestSol;
    bestSol = new Solution(instance);
    FOR(i, bestInteger)
      bestSol->InsertBin(new Pattern());
  }
...
void Model::EnableDualIneq(){
  ...
  lastObj = safeObj;
  ld multiW = DD(safeObj) / L0;
  FOR(i, N){
    GRBColumn col;
    col.addTerm(1, model->getConstrByName("u" + to_string(i)));
    ld RHS = ceil(multiW * weights[i] * 1e9) / binCapacity / 1e9;
