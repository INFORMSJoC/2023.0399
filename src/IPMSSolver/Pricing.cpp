...
Pricing::Pricing(CPM* cpm, vi& weights, vi& freq, vi& wToIt, vi& conflictIts, vll& v, vector<vc>& compatible):
    weights(weights), freq(freq), wToIt(wToIt), conflictIts(conflictIts),
    v(v), vSR(cpm->dualSR), itsToSR(cpm->itsToSR), setsSR(cpm->setsSR), compatible(compatible),
    seenSR(cpm->seenSR), W(binCapacity), residueAccept(binCapacity - 1){}
...
vector<Pattern*> Pricing::getFromPool(){
  ...
  for(auto pat : pool){
    if(pat->residue() > residueAccept || pat->cap > binCapacity)
      continue;
...
bool Pricing::UpdateDPsize(int N, bool force){
  if (N > SZ(dp) || force){
    ll dpSize = N  * (W + 1);
